...
for line in file:
    ...
    time_near_the_machine = str(datetime.timedelta(hours=hours_in, minutes=minutes_in) + datetime.timedelta(hours=0, minutes=refueling_time))[:-3]
...
while main_time != datetime.timedelta(hours=24, minutes=0):
    if str(main_time)[:-3] in departure_time:
        ...
        print('В', str(main_time)[:-3], 'клиент', departure_time[str(main_time)[:-3]][0], departure_time[str(main_time)[:-3]][1],
               departure_time[str(main_time)[:-3]][2], departure_time[str(main_time)[:-3]][3], 'заправил свой автомобиль и покинул АЗС')
        for i in range(len(machine_with_max_quantity)):
            print('Автомат №', i + 1, ' ', ' максимальная очередь: ', machine_with_max_quantity[i + 1],
                  ' Марки бензина: ', machine_with_petrol[i + 1], ' ', ' -> ', '*' * quantity_people_near_machine[i + 1], sep='')
    ...
    if str(main_time)[:-3] in arrival_time:
        ...
        while i != (len(petrol_with_machine[arrival_time[str(main_time)[:-3]][1]])):
            if quantity_people_near_machine[petrol_with_machine[arrival_time[str(main_time)[:-3]][1]][i]] != machine_with_max_quantity[petrol_with_machine[arrival_time[str(main_time)[:-3]][1]][i]]:
        ...
        if machine == 0:
            print('В', str(main_time)[:-3], 'новый клиент:', arrival_time[str(main_time)[:-3]][0], arrival_time[str(main_time)[:-3]][1],
                  arrival_time[str(main_time)[:-3]][2], arrival_time[str(main_time)[:-3]][3], 'не смог заправить свой автомобиль и покинул АЗС')
            for i in range(len(machine_with_max_quantity)):
                print('Автомат №', i + 1, ' ', ' максимальная очередь: ', machine_with_max_quantity[i + 1],
                      ' Марки бензина: ', machine_with_petrol[i + 1], ' ', ' -> ', '*' * quantity_people_near_machine[i + 1], sep='')
            print(1)
            ...
            print('В ', str(main_time)[:-3], ' ', 'новый клиент ', arrival_time[str(main_time)[:-3]][0], ' ', arrival_time[str(main_time)[:-3]][1], ' ',
                  arrival_time[str(main_time)[:-3]][2], ' ', arrival_time[str(main_time)[:-3]][3], ' ', 'встал в очередь к автомату №', machine, sep='')
            for i in range(len(machine_with_max_quantity)):
                print('Автомат №', i + 1, ' ', ' максимальная очередь: ', machine_with_max_quantity[i+1], ' Марки бензина: ',
                      machine_with_petrol[i+1], ' ', ' -> ', '*' * quantity_people_near_machine[i+1], sep='')
